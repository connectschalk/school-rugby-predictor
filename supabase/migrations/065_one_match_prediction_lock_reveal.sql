-- Lock-to-reveal: only locked predictions are visible to others; viewer always sees own row via RPC.

alter table public.one_match_predictions
  add column if not exists is_locked boolean not null default false;

comment on column public.one_match_predictions.is_locked is
  'When true, prediction is visible in public preview; anon reads go through get_one_match_predictions_visible.';

-- Anon/authenticated clients use this instead of direct table select (RLS tightened below).
...
revoke all on function public.get_one_match_predictions_visible(text, text) from public;
grant execute on function public.get_one_match_predictions_visible(text, text) to anon, authenticated;
...
begin
  ...
  update public.one_match_predictions
  set is_locked = true,
      updated_at = now()
  where id = v_id;
...
revoke all on function public.lock_one_match_prediction(text, text) from public;
grant execute on function public.lock_one_match_prediction(text, text) to anon, authenticated;

-- Block direct table reads for public; admins keep full access via omp_select_admin.
...
-- Re-apply admin policy (unchanged semantics).
...
-- Reject edits after lock (same RPC signature as before).
