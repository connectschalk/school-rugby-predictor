-- Soccer knockout penalty shootout: predictions, results, and scoring.
...
alter table public.user_predictions
  drop constraint if exists user_predictions_predicted_penalty_winner_check;

alter table public.user_predictions
  add constraint user_predictions_predicted_penalty_winner_check
  check (predicted_penalty_winner is null or predicted_penalty_winner in ('home', 'away'));

alter table public.game_matches
  drop constraint if exists game_matches_penalty_winner_check;

alter table public.game_matches
  add constraint game_matches_penalty_winner_check
    ...
    (
      ...
      and (
        (
          ...
          and predicted_penalty_winner is null
        )
        or (
          predicted_home_score = predicted_away_score
          and (
            predicted_penalty_winner is null
            or predicted_penalty_winner in ('home', 'away')
          )
        )
      )
    )
...
comment on column public.user_predictions.predicted_penalty_winner is
  'Soccer knockout draw: predicted penalty-shootout winner (home or away).';
comment on column public.game_matches.penalty_winner is
  'Soccer knockout draw: team that won the penalty shootout (home or away).';
...
begin
  ...
  if v_scoring_mode = 'soccer_exact_score' then
    ...
    select
      ...
      (
        case
          when v_actual_winner = 'draw' and v_penalty_winner is not null then
            coalesce(up.predicted_penalty_winner, up.predicted_winner) = v_penalty_winner
        ...
        when v_actual_winner = 'draw' and v_penalty_winner is not null then v_penalty_winner
