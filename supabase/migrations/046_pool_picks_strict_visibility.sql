-- Pool picks: no user_predictions (or profile rows for the picks list) are returned until the game gate opens.
-- Removes per-viewer "early unlock" via own locked prediction. Non-members still get zero rows (is_pool_member).
