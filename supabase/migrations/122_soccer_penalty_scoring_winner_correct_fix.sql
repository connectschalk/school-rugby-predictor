-- Fix soccer penalty scoring: winner_correct must never be NULL (legacy 1–1 predictions
-- without predicted_penalty_winner were causing NOT NULL violations on insert).
    ...
    select
      ...
      case
        when v_actual_winner = 'draw' and v_penalty_winner is not null then
          (
            up.predicted_penalty_winner is not null
            and up.predicted_penalty_winner = v_penalty_winner
          )
          or (
            up.predicted_penalty_winner is null
