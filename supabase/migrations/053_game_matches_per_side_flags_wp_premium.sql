-- Per-side prestige / WP elite flags from Teams tab; WP Premium group for pool linking (has_wp_elite_team).

alter table public.game_matches
  add column if not exists home_is_prestige_team boolean not null default false,
  add column if not exists away_is_prestige_team boolean not null default false,
  add column if not exists home_is_wp_elite boolean not null default false,
  add column if not exists away_is_wp_elite boolean not null default false;

insert into public.fixture_groups (name, slug, group_type, is_active)
values ('WP Premium', 'wp-premium', 'custom', true)
...
as $$
  select exists (
    ...
    where fg.id = p_fixture_group_id
      and fg.slug not in ('prestige-pool', 'interprovincial', 'wp-elite', 'wp-premium')
...
after insert or update of
  ...
  home_team_province,
  away_team_province,
  home_is_prestige_team,
  away_is_prestige_team,
  home_is_wp_elite,
  away_is_wp_elite
