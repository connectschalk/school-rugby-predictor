-- Align submit_memory_story RPC signature with the contributor frontend (named parameters).
-- Drops legacy overloads from 098/102 and recreates a single function with defaults.
...
begin
  ...
  v_title := coalesce(
    ...
    nullif(left(trim(coalesce(p_description, '')), 80), '')
  );
  ...
  insert into public.memory_stories (
    ...
    v_pin_id, v_title, trim(p_description), coalesce(p_story_type, 'mixed'),
