...
function findAllPathsWithWeights(
  ...
  function dfs(
    ...
    if (current === target && path.length > 0) {
      ...
      const baselineCount = namesInPath.filter((name) =>
        BASELINE_TEAMS.has(name)
      ).length
...
export default function ResultsPage() {
  ...
  const teamAName = getTeamName(teams, teamA)
  const teamBName = getTeamName(teams, teamB)
  ...
  function runPrediction() {
    ...
    const directMatch = matches.find(
      (m) =>
        (String(m.team_a_id) === teamA && String(m.team_b_id) === teamB) ||
        (String(m.team_a_id) === teamB && String(m.team_b_id) === teamA)
    )

    if (directMatch) {
      const margin =
        String(directMatch.team_a_id) === teamA
      ...
      setResult({
        ...
        pathCount: 1,
        confidence: getConfidence('direct', 1, 1),
        paths: [],
        directMatch,
      })
      return
    ...
    const weightedTotal = allPaths.reduce(
      (sum, p) => sum + p.totalMargin * p.weight,
      0
    )
    ...
    <main className="min-h-screen bg-white text-black">
      <div className="mx-auto max-w-5xl px-6 py-12">
        ...
        <p className="mt-2 text-gray-600">
          Uses all connected paths up to 5 links in the selected season. Baseline schools and more
          consistent teams carry more weight in indirect predictions.
        </p>
                ...
                <div className="mt-6 space-y-6">
                  <div className="rounded-2xl border border-gray-200 bg-gray-50 p-6">
                    ...
                    {result.type === 'direct' && result.directMatch && (
                      <p className="text-sm text-gray-600">
                        Match date: {new Date(result.directMatch.match_date).toLocaleDateString()}
                      </p>
                    )}
                  </div>
                          ...
                          >
                            <p className="font-medium">
                              ...
                              {pathResult.totalMargin > 0
                                ? `${teamAName} by ${Math.abs(pathResult.totalMargin)}`
                                : pathResult.totalMargin < 0
                                ? `${teamBName} by ${Math.abs(pathResult.totalMargin)}`
                                : 'Draw'}
                            </p>

                            <p className="mt-1 text-sm text-gray-600">
                              Links: {pathResult.path.length} | Weight: {pathResult.weight.toFixed(3)} |
                              Baseline teams in path: {pathResult.baselineCount} | Consistency factor:{' '}
