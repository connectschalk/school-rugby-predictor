...
export default function NetworkPage() {
  ...
  const filteredMatches = useMemo(() => {
    ...
    const baselineId = Number(baselineTeam)
    const maxDepth = Number(depth)

    const { reachableTeamIds } = getReachableTeamsFromBaseline(
      matches,
      baselineId,
      maxDepth
    )
    ...
    return matches.filter(
      (m) => reachableTeamIds.has(m.team_a_id) && reachableTeamIds.has(m.team_b_id)
    )
  ...
  const graphData = useMemo(() => {
    ...
    const links: GraphLink[] = filteredMatches.map((match) => {
      ...
      return {
        ...
        label: `${teamA} ${match.team_a_score} - ${match.team_b_score} ${teamB} | margin ${
          margin > 0 ? '+' : ''
        }${margin}`,
        matchId: match.id,
            ...
            <div className="mt-8 rounded-2xl border border-gray-200 p-4 shadow-sm">
              ...
              >
                <ForceGraph2D
                  ...
                  nodeLabel={(node: any) =>
                    `${node.name} | Pool ${node.poolId} | Score ${
                      node.relativeScore > 0 ? '+' : ''
                  ...
                  onNodeClick={(node: any) => {
                    setSelectedInfo(
                      `Team: ${node.name} | Pool: ${node.poolId} | Relative score: ${
                        node.relativeScore > 0 ? '+' : ''
