...
import type { TeamsRegistryDebug, TeamsRegistryUnresolvedTeam } from '@/lib/sheet-teams-registry'
...
function teamsRegistryDebugFromRunSummary(summary: unknown): TeamsRegistryDebug | null {
  ...
  if (typeof o.teams_csv_url_used !== 'string') return null
  if (typeof o.teams_rows_count !== 'number') return null
  if (!Array.isArray(o.first_5_canonical_names)) return null
  if (typeof o.has_lookup_heidelberg_volkskool !== 'boolean') return null
  if (typeof o.has_lookup_hugenote_welkom !== 'boolean') return null
  if (!Array.isArray(o.all_lookup_keys_containing_heidelberg)) return null
  if (!Array.isArray(o.all_lookup_keys_containing_hugenote)) return null
  if (!Array.isArray(o.unresolved_teams)) return null

  const unresolved: TeamsRegistryUnresolvedTeam[] = []
  for (const u of o.unresolved_teams as unknown[]) {
    if (!u || typeof u !== 'object') continue
    const x = u as Record<string, unknown>
    const row = typeof x.fixture_sheet_row === 'number' ? x.fixture_sheet_row : Number(x.fixture_sheet_row)
    const side = x.side
    const rawTeam = x.raw_team_value
    const nk = x.normalized_team_key
    const sim = x.similar_lookup_keys
    if (!Number.isFinite(row) || (side !== 'home' && side !== 'away')) continue
    if (typeof rawTeam !== 'string' || typeof nk !== 'string') continue
    if (!Array.isArray(sim)) continue
    unresolved.push({
      fixture_sheet_row: row,
      side,
      raw_team_value: rawTeam,
      normalized_team_key: nk,
      similar_lookup_keys: sim.map(String),
    })
  }
  ...
  const out: TeamsRegistryDebug = {
    ...
    has_lookup_heidelberg_volkskool: o.has_lookup_heidelberg_volkskool,
    has_lookup_hugenote_welkom: o.has_lookup_hugenote_welkom,
    all_lookup_keys_containing_heidelberg: o.all_lookup_keys_containing_heidelberg.map(String),
...
export default function AdminPage() {
  ...
  const [bulkScoreCompletedBusy, setBulkScoreCompletedBusy] = useState(false)
  const [bulkScoreCompletedMessage, setBulkScoreCompletedMessage] = useState('')
  ...
  async function runBulkScoreCompletedUnscored() {
    setBulkScoreCompletedBusy(true)
    setBulkScoreCompletedMessage('')
    const {
      data: { session },
    } = await supabase.auth.getSession()
    const token = session?.access_token
    if (!token) {
      setBulkScoreCompletedMessage('Not signed in.')
      setBulkScoreCompletedBusy(false)
      return
    }
    ...
    if (!res.ok || !json.ok) {
      setBulkScoreCompletedMessage(json.error ?? 'Scoring failed.')
      setBulkScoreCompletedBusy(false)
      return
    }
    setBulkScoreCompletedMessage(
      `Scored ${json.matches_scored_ok ?? 0} match(es); attempted ${json.match_ids_attempted ?? 0}.`
    )
    void loadScoringSummary()
    setBulkScoreCompletedBusy(false)
  }
          ...
          <div className="mt-3 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm md:p-6">
            ...
            <div className="mt-5 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-800">
              ...
              <p className="mt-1 text-xs text-gray-600">
                Sheet sync imports fixtures only; it does not score predictions. Use{' '}
                <strong className="font-medium text-gray-800">Score all completed unscored games</strong> below when you
                are ready (same RPC as per-fixture &quot;Run scoring&quot;). Refresh the summary after scoring or sync.
              </p>
              {scoringSummaryLoading ? (
                ...
                <ul className="mt-2 list-inside list-disc space-y-0.5 text-xs text-gray-700">
                  <li>
                    ...
                    <code className="rounded bg-gray-100 px-1">user_prediction_scores</code> rows yet (run bulk scoring
                    below after sync; otherwise check warnings or run scoring on the fixture).
                  </li>
                  <li>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => void runBulkScoreCompletedUnscored()}
                  disabled={bulkScoreCompletedBusy}
                ...
                >
                  {bulkScoreCompletedBusy ? 'Working…' : 'Score all completed unscored games'}
                </button>
              </div>
              {bulkScoreCompletedMessage ? (
                <p className="mt-2 text-xs text-gray-700">{bulkScoreCompletedMessage}</p>
              ) : null}
              <div className="mt-4 border-t border-gray-200 pt-4">
                <p className="font-semibold text-gray-900">Fixture group links</p>
                <p className="mt-1 text-xs text-gray-600">
                  Every <strong className="font-medium text-gray-800">completed</strong> match can have{' '}
                  <code className="rounded bg-gray-200 px-1">game_match_groups</code> rebuilt from stored context (league,
                  tournament when set, Interprovincial, Prestige, WP Premium, provinces, legacy{' '}
                  <code className="rounded bg-gray-200 px-1">province_group</code>). Sheet sync does not run this; use
                  the buttons when you need links or pool visibility updated.
                </p>
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  <button
                    type="button"
                  ...
                  <button
                    ...
                    onClick={() => void runRepairFixtureGroupLinks()}
                    disabled={repairGroupLinksBusy}
                    className="rounded-lg border border-gray-900 bg-white px-3 py-1.5 text-xs font-semibold text-gray-900 hover:bg-gray-50 disabled:opacity-50"
                  >
                    {repairGroupLinksBusy ? 'Working…' : 'Repair all fixture group links'}
                  </button>
                </div>
            ...
            <div ref={masterSyncWarningsRef} id="master-sync-warnings" className="mt-4 scroll-mt-24">
              <MasterSheetSyncWarningsPanel
                ...
                defaultOpen={effectiveSyncWarnings.length > 0 || !!teamsRegistryPreviewDebug}
                title="Warnings / errors (latest run)"
                teamsRegistryDebug={teamsRegistryPreviewDebug}
              />
            </div>
                  ...
                  <div className="relative z-[1] flex max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl">
                    ...
                    <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-4 pt-2 md:px-4">
                      <MasterSheetSyncWarningsPanel
                        ...
                        teamsRegistryDebug={teamsRegistryDebugFromRunSummary(
                          syncRuns.find((x) => x.id === syncRunDetailId)?.summary
                        )}
                      />
                    </div>
                  </div>
