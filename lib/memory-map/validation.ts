...
export function resolveMemoryTitle(
  ...
  const place = pinTitle?.trim()
  if (place) return `${y} memory at ${place}`
  return `Memory ${y}`
}

export function resolveMemoryDescription(
  shortNote: string,
  textMemory: string,
  title: string,
  hasMedia: boolean
): string {
  const note = shortNote.trim() || textMemory.trim()
  if (note) return note
  if (hasMedia && title.trim()) return title.trim()
  return ''
}

export function isAutoGeneratedMemoryTitle(title: string, pinTitle: string | undefined, year: string): boolean {
  const y = year.trim()
  const place = pinTitle?.trim()
  if (!y) return false
  if (place && title === `${y} memory at ${place}`) return true
  return title === `Memory ${y}`
}
...
export function validateQuickContributorSubmit(input: QuickContributorSubmitInput): string | null {
  ...
  const title = resolveMemoryTitle(
    input.memoryTitle,
    input.shortNote,
    input.textMemory,
    input.year,
  ...
  const description = resolveMemoryDescription(
    input.shortNote,
    input.textMemory,
    title,
    hasMedia
  )
  if (!title.trim()) return 'Add the year this happened.'
  if (!hasMedia && !description.trim()) return errors.note ?? 'Write the memory.'
