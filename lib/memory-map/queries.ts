...
export async function fetchMemoryMapBundleBySlug(slug: string): Promise<MemoryMapBundle | null> {
  ...
  if (!client) return demo ? enrichBundle(demo) : null
  ...
  try {
    ...
    const [areasRes, categoriesRes, pinsRes, storiesRes, tagsRes] = await Promise.all([
      ...
      client
        ...
        .in(
          'area_id',
          (
            await client.from('memory_areas').select('id').eq('memory_map_id', mapId)
      ...
      map: {
        ...
        organisation: org
          ? {
              ...
              type: org.type as MemoryMapBundle['map']['organisation'] extends infer O
    ...
    if (bundle.areas.length === 0 && slug === DEMO_MAP_SLUG) {
      return enrichBundle(DEMO_MEMORY_MAP_BUNDLE)
    }
...
export async function fetchAdminMemoryMapBundle(mapId: string): Promise<MemoryMapBundle | null> {
  const demo = mapId === DEMO_MEMORY_MAP_BUNDLE.map.id ? DEMO_MEMORY_MAP_BUNDLE : getDemoBundle(DEMO_MAP_SLUG)
  const client = createMemoryMapServerClient()
  ...
  try {
    ...
    if (!map) return demo ? enrichBundle(demo) : null
