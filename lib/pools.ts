...
import {
  fetchPoolInviteByToken as fetchPoolInviteByTokenServer,
  parsePoolInviteRow,
  type PoolInvitePreview,
  type PoolInviteViewerState,
} from '@/lib/pool-invite-server'
import {
...
export type { PoolInvitePreview, PoolInviteViewerState }
export { parsePoolInviteRow }
...
export async function fetchPoolInviteByToken(
  ...
  const { pool, error } = await fetchPoolInviteByTokenServer(client, token, invitedByUserId)
  return { pool, error: error ? { message: error.message } : null }
}
